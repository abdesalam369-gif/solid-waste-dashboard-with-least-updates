    ...
    return (
        ...
        >
            ...
            <div className={`mb-3 sm:mb-5 p-3 sm:p-5 rounded-2xl sm:rounded-[1.8rem] transition-all duration-700 ease-out ${color.replace('text', 'bg').replace('600', '50').replace('500', '50').replace('700', '50').replace('800', '50')} dark:bg-slate-800/80 group-hover:scale-110 sm:group-hover:scale-115 group-hover:rotate-12 group-hover:shadow-lg transform shadow-inner`}>
                <span className="text-2xl sm:text-4xl filter drop-shadow-sm transition-transform duration-500 group-hover:scale-110">{icon}</span>
            </div>
            
            <div className={`text-lg sm:text-3xl font-black mb-1 sm:mb-2 tracking-tighter transition-all duration-500 group-hover:scale-105 ${color}`}>
                {value}
            </div>
            
            <div className="text-[9px] sm:text-[11px] font-black text-slate-400 dark:text-slate-500 leading-tight mb-2 sm:mb-4 uppercase tracking-[0.1em] sm:tracking-[0.15em] group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors">
                {label}
            </div>
            ...
            {comparisonValue !== undefined && (
                <div className="mt-auto pt-3 sm:pt-4 border-t border-slate-50 dark:border-slate-800 w-full group-hover:border-indigo-50 dark:group-hover:border-indigo-900/30 transition-colors">
                    <div className="text-[9px] sm:text-[10px] text-slate-400 dark:text-slate-500 font-bold flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1.5">
